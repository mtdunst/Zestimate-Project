...
reg1 <- lm(price ~ ., data = st_drop_geometry(sales_external_data.sf) %>% 
                                 dplyr::select(price, park_nn1, NUM_BATHS.cat, heatedarea, npa, Quality, Age, NUM_BEDS.cat, numfirepla, HH_inc, college_perc, NUM_FLOORS.cat))
...
bothRegressions <- 
  rbind(
    dplyr::select(charlotte.test, starts_with("price"), Regression, Name) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, price.Error)),
    dplyr::select(charlotte.test.nhood, starts_with("price"), Regression, Name) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, price.Error)))    
```
