...
ggplot() +
  geom_sf(data = schools.sf, aes(fill=factor(Rank), color=factor(Rank))) +
...
reg1 <- lm(price ~ ., data = st_drop_geometry(sales_external_data.sf) %>% 
                                 dplyr::select(price, park_nn1, park_nn2, park_nn3, fullbaths, heatedarea, npa, Rank, Age, bedrooms, numfirepla, halfbaths, HH_inc, college_perc))
